...
Each cell stores a log-odds value:
  - 0.0  = unknown (50% probability)
  - > 0  = likely occupied
  - < 0  = likely free

Bresenham raycasting marks cells along each LIDAR ray as free,
and the endpoint as occupied (if hit detected).
"""
...
class OccupancyGrid:
    """2D occupancy grid for one drone (or fused result)."""

    def __init__(self, width_m=30.0, height_m=20.0, resolution=0.25,
                 origin_x=-15.0, origin_y=-10.0):
        """
        Args:
            width_m:    grid width in meters  (default 30 = warehouse width)
            height_m:   grid height in meters (default 20 = warehouse depth)
            resolution: meters per cell (0.25 -> 120x80 grid)
            origin_x:   world X of bottom-left corner
            origin_y:   world Y of bottom-left corner
        """
        ...
        self.cols = int(width_m / resolution)   # 120
        self.rows = int(height_m / resolution)  # 80
    ...
    def update_from_scan(self, drone_x, drone_y, scan_points):
        """
        Update grid from one LIDAR sweep.

        Args:
            drone_x, drone_y: drone position in world coords
            scan_points: list of (hit_x, hit_y, is_hit)
                is_hit=True  -> obstacle at that point
                is_hit=False -> ray reached max range (all free)
        """
